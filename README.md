...
**无损升级（保留练习进度与全部配置）**：升级只换程序本体，新角色 / 新题库随新版本自动补齐；你的进度与配置按下面任选一种方式带过去——

- **要保留的「四样」**：`data/`（全部练习记录：会话 / 收藏夹 / 错题本 / 复盘报告 / 打卡统计）、`.env`（API Key 与接口地址）、`config.local.yaml`（⚙️ 设置里改过的内容：模型 / 音色 / 手机访问等）、`materials/` 里你自己加的文件（导入的简历 `profile.md`、自建题库 / 角色）。
- **方式 A · 最稳（推荐）**：新版解压到**新文件夹** → 把旧文件夹的 `data/`、`.env`、`config.local.yaml`（及 `materials/` 自加文件）复制过去 → 启动新程序（首次启动自动补齐新增题库 / 角色）→ 核对：⚙️ 设置里版本号 = 新版本、首页能看到旧记录。
- **方式 B · 最省事**：新版**直接覆盖解压**到旧文件夹再启动——v0.16.1 起会自动补齐缺失的新题库 / 角色，**不会覆盖你的任何文件**；进度、Key、设置、自定义素材原样保留。
- **避免**：删掉旧文件夹再解压新版 = 重置（进度与配置一起丢）；想彻底重来就这么做，想保留请先备份上面四样。
- 📖 图文详解：使用手册「**6 更新与无损升级**」（[docs/USER-GUIDE.pdf](docs/USER-GUIDE.pdf)）。
